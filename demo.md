...
**Note:** the following steps are for macOS.
...
`docker exec shard1a mongosh --port 27018 --eval "rs.status()" 2>&1 | grep -iE "(primary|secondary|host)"`
4. Stop one secondary node:
`docker stop shard1c`
5. Verify the cluster still works:
`docker exec mongos mongosh --port 27017 --eval 'db.getSiblingDB("vesselDB").ais_data.countDocuments({})'`
6. Check what changed in replica set:
`docker exec shard1a mongosh --port 27018 --eval "rs.status()" 2>&1 | grep -iE "(primary|secondary|host|_id)"`
